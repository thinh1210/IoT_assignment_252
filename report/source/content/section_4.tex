...
\begin{itemize}
    \item Board: ESP32-S3 (Yolo UNO)
    \item Framework: Arduino + FreeRTOS (PlatformIO)
    \item Cảm biến: DHT11
    \item Hiển thị: OLED SH1106 (U8g2)
    \item Network:
    \begin{itemize}
...
Kết quả định lượng hiện có là \textbf{accuracy offline} (train/validation) từ pipeline huấn luyện. Ở mức firmware, nhóm đã xác nhận được tính đúng đắn luồng suy luận (functional correctness) qua log và hành vi actuator. Để tăng tính học thuật cho phần ``hardware accuracy'', nhóm đề xuất bước tiếp theo là thêm test harness replay tập mẫu có nhãn trực tiếp trên board và tự động tính confusion matrix tại runtime.
