...
struct ButtonState {
    ...
    uint32_t clickTimeoutMs; // Thời gian chờ để xác định là click đơn/đôi/ba (mặc định 300ms)
    ...
    ButtonState(uint8_t p, uint32_t lpMs, bool al) 
        : pin(p), activeLow(al), longPressTimeMs(lpMs), debounceTimeMs(50), 
          clickTimeoutMs(300), 
          lastReading(al ? HIGH : LOW), currentState(false), 
...
public:
    ButtonHandler();
    void addButton(uint8_t pin, uint32_t longPressMs = 2000, bool activeLow = true);
    void setSemaphore(SemaphoreHandle_t semaphore);
    void begin();
    void loop();

    // Hàm đọc sự kiện: Trả về index của nút có sự kiện, và loại sự kiện vào tham số event
